...
An app for exploring 20 years of the [RetroMacCast](https://retromaccast.libsyn.com) podcast (James & John) — search, browse, relive every episode's transcript and show notes, watch the show's YouTube videos in a faithful QuickTime Player-style jukebox, and boot a real classic Mac, all wrapped in a UI that pays homage to the classic Mac look. Being built with the intent to offer it to the hosts as the show's official app.
...
# Fetch the show's YouTube channel uploads (YouTube Data API v3) and upsert them into the
# corpus -- title, publish date, thumbnail URL, duration, and a best-effort episode number
# match (regex against the title) for jukebox playback in the app's Videos tab. Plain upsert
# against the video id, so re-running only ever adds/refreshes rows, never duplicates.
.build/release/rmc-pipeline sync-videos [--channel-handle @RetroMacCast] [--limit N]
...
`sync-videos` is the other one that doesn't touch the Claude API at all — it needs a `YOUTUBE_API_KEY` environment variable (a YouTube Data API v3 key from Google Cloud Console) instead, and re-running it is a plain upsert against each video's own YouTube id, so it's always safe to re-run in full rather than needing an incremental `--since` flag.
...
- `videos` — one row per YouTube upload synced by `sync-videos` (YouTube video id as primary key, title, publish date, thumbnail URL, duration, a best-effort `episodeId` regex-matched from the title — `NULL` for anything that isn't a numbered episode recording, like bonus footage or live streams — and `syncedAt`); the app's Videos tab groups these by year for its picker
...
- **`App/`** — `RetroMacCastApp.swift` (entry point, owns the shared `CorpusUpdateManager` and `AppearanceManager`), `RootTabView.swift` (the five-tab shell — Home, Museum, Emulators, Videos, Trivia; also drives `.preferredColorScheme` from the selected desktop theme — see "Classic Mac visual chrome" below), `RMCBadge.swift` (an original circular podcast-badge illustration, inspired by the show's real logo composition but not a copy of it — see that section for why)
...
- **`Videos/`** — the **Videos** tab: a QuickTime Player-styled jukebox for the show's YouTube channel (see "The QuickTime Player video jukebox" below for the full chrome writeup) — `QuickTimePlayerChrome.swift` (the window chrome + Standard Movie Controller bar), `YouTubePlayerModel.swift` (drives the real YouTube IFrame Player API via a `WKWebView` + JS message bridge, not a bare embed), `VideosView.swift` (the year/video pickers and surrounding controls below the player)
...
## The QuickTime Player video jukebox

The Videos tab (`Videos/QuickTimePlayerChrome.swift`, `VideosView.swift`) is a from-scratch recreation of the classic Mac OS "Standard Movie Controller" — the chrome around a real 1997 QuickTime Player window, playing the show's actual YouTube uploads underneath it. Every element was checked against a real reference photo of a physical Mac running QuickTime Player, not guessed:

- **Title bar** — the same solid-black 1px-pinstripe title bar as the rest of the app's chrome, with functional close/zoom boxes isolated from the pinstripe by a small solid-white margin so the stripes terminate cleanly at each box's edge instead of running into it.
- **Standard Movie Controller bar** — a flat white 24px bar (not a checkerboard — see below) holding, left to right: a speaker button (click to reveal a volume slider), a play/pause button, the scrubber, frame-step buttons, and a decorative badge box. Every button is its own bordered white box via a shared `.buttonBoxBorder()` modifier, laid out with zero spacing so adjacent buttons' borders read as one continuous line rather than a double gap.
- **The scrubber** fills its **played** portion with the same dense black/white checkerboard dither used elsewhere in the app's chrome (`DitheredPattern`, originally built for the Finder-style scrollbar), thermometer-style, while the **unplayed** remainder and the bar's own background stay flat white — matching how a real QuickTime scrubber fills the elapsed portion of the track, not just a thin position marker on an otherwise uniform groove. The thumb itself is a bordered white box with a few vertical grip ridges, dragging live (`DragGesture`) with `model.seek(to:)` firing once on release, same shape as the inline player's own scrubber in `Search/SearchView.swift`.
- **Real YouTube playback**, not a bare `<iframe>` — `YouTubePlayerModel.swift` bootstraps the actual `youtube.com/iframe_api` inside a `WKWebView` with a JS↔Swift message bridge, so the custom controller bar above can genuinely drive and observe play/pause/seek/volume state instead of just displaying YouTube's own default controls (`controls: 0`). Selecting a video autoplays immediately — `mediaTypesRequiringUserActionForPlayback = []` relaxes WebKit's in-page-gesture requirement, and an explicit `player.playVideo()` call inside `onReady` triggers it, since the `autoplay` playerVar alone isn't reliably sufficient. Closed captions default off (a `CAPTIONS` checkbox turns them on) — enforced via the IFrame API's `onApiChange` hook plus a timeout fallback, since `cc_load_policy: 0` alone isn't reliable when the captions module attaches after the ready/state-change events already fired.
- **A two-level Year → Video picker** below the player, sharing the same custom pop-up-menu chrome (white background, hard black offset shadow, Chicago font, black-bar hover invert) as the rest of the app's dropdowns — picking a year scopes the second pop-up to just that year's uploads, split into "Episode Recordings" and "Bonus & Extras" sections.
- **1-BIT VIDEO** checkbox applies a `grayscale/contrast` CSS filter to the actual YouTube embed plus a dense scanline+dither `Canvas` overlay, scoped strictly to the video canvas (`content` inside `QuickTimePlayerChrome`) — the title bar and controller bar keep rendering normally regardless of the toggle.
- **Idle "off-air" state** — before any video is picked, the full chrome (title bar + controller bar) is still visible, with a bundled SMPTE color-bars test-pattern image (`VideosPlaceholder` asset) filling the video area instead of a bare empty box.

**A genuine rendering bug worth knowing about if this chrome is touched again**: a single opaque `Color.white` fill (as a `Shape.fill`, as a `Canvas` fill, and as several smaller `Color.white` pieces tiled edge-to-edge to recombine into one contiguous run — all three were tried) silently fails to render anywhere in this bar once its width crosses roughly 400–500pt, confirmed by swapping the identical code to `Color.red` at the identical width, which rendered correctly every time, and cross-checked through two independent screenshot capture pipelines to rule out a capture artifact. The root cause was never fully pinned down (suspected some SwiftUI/Core Animation large-solid-region compositing optimization), but the reliable workaround — used for both the bar's own flat-white background and the scrubber groove's unplayed portion — is drawing "white" as `DitheredPattern` at a near-zero tint (`.black.opacity(0.05)`) instead of a literal solid fill. `DitheredPattern`'s own checkerboard already proved out at ~850pt (the full bar width) without issue, so routing through it sidesteps whatever the actual bug is rather than needing to fully diagnose it.
...
1. **`.github/workflows/weekly-sync.yml`** runs every Monday (plus a manual `workflow_dispatch` trigger) on a `macos-latest` runner: downloads the most recently published corpus, runs `crawl-index` → `resolve-audio` → `transcribe-batch` → `classify` → `reset-stale-synthesis` → `synthesize` → `sync-videos` → `export-manifest`, and — only if the episode count *or* the video count actually changed — publishes a new GitHub Release tagged `corpus-YYYY-MM-DD` with `rmc.sqlite` and `manifest.json` as assets. Both `ANTHROPIC_API_KEY` and `YOUTUBE_API_KEY` live only as repo secrets; neither ever touches app code or a shipped binary.
...
5. **`App/RetroMacCast/Resources/rmc.sqlite`** is a bundled fallback for a fresh install with nothing downloaded yet (`Corpus.init()` uses it, read-only, whenever `~/Library/Application Support/RetroMacCast/rmc.sqlite` doesn't exist). It's gitignored (100MB+) and has no CI step that refreshes it — it's a manual local copy, so it silently goes stale relative to the live schema/data every time a migration or a new mining pass ships. **After any schema change, or periodically to keep fresh-install content current, download the latest release's `rmc.sqlite` and overwrite this file by hand**: `gh release download --repo 7thefcpguy1/retromaccast-archive --pattern rmc.sqlite --clobber -O App/RetroMacCast/Resources/rmc.sqlite` (run from the repo root; the tag defaults to the newest release marked "Latest").

## Status (as of this writing)

- **Pipeline**: complete and self-sustaining. 742 episodes transcribed, 0 failures, classified into 61 collections across 10 Museum categories, 94 trivia facts mined (every episode-linked one anchored to a real moment), 253 YouTube videos synced (223 matched to a numbered episode, the rest bonus/live-stream footage); the weekly sync job keeps all of this current going forward without manual intervention.
- **App**: Home (search + on-this-day), Museum (full System 7-chrome browsing experience), Emulators (real classic Mac OS via Infinite Mac), Trivia (rotating facts with jump-to-moment playback), and Videos (QuickTime Player-styled YouTube jukebox — see "The QuickTime Player video jukebox" above) tabs are all built and verified on both iOS and macOS. Settings (Updates, Background theme picker, About) is built and verified on macOS.
- **Weekly sync**: live. Repo is [7thefcpguy1/retromaccast-archive](https://github.com/7thefcpguy1/retromaccast-archive) (public), `ANTHROPIC_API_KEY` is configured as a repo secret, a baseline release (`corpus-2026-08-05`, the current 742-episode corpus) has been published so future runs only ever process new episodes incrementally, and the workflow has been triggered manually to confirm it runs end-to-end. `sync-videos` and the `YOUTUBE_API_KEY` secret it needs were added to the workflow after that baseline — worth confirming the secret is actually set in the repo's Actions settings before relying on the next scheduled run to pick up new videos.

## Open items

- **Reach out to James & John before going much further** — reached out; coordinating a time to demo the app to them on Zoom. Their own collection photos would still beat any stock/Wikimedia imagery for illustrating specific models, and their actual logo file — once they're comfortable sharing it — should replace `RMCBadge.swift`'s original placeholder illustration.
...
- Museum's category/product cascade windows can't be re-clamped fully into view if the app window is resized smaller while one is open (only clamped once, at open time) -- a fix was attempted and reverted after live testing found it introduced a worse cross-level positioning bug when both cascade levels are open at once. Needs a dedicated pass that coordinates the two levels' corrections rather than running them independently.
